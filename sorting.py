    ...
    for pos in range(size):
        ...
        for seek in range(pos,size):    # find smallest in range
            ...
            if numList[seek] < numList[minpos]:
                minpos = seek  # update the index of the minimum element
...
def insertion_sort(numList):
    ...
    for pos in range(1,size):
        ...
        # -- YOUR CODE STARTS HERE
        j = pos - 1  # start by comparing with the previous element
        while j >= 0 and numList[j] > value:
            numList[j + 1] = numList[j]  # shift the larger element one position to the right
            j -= 1
        numList[j + 1] = value  # insert the value at the correct position
    return numList
    ...
    while not sorted:
        ...
        for pos in range(1,size):
            if numList[pos] < numList[pos-1]:
                sorted = False  # mark list as unsorted
                # -- YOUR CODE STARTS HERE
